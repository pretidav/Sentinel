...
people = {'DAVID':False,'ROBERTA':False,'MARIO':False,'MATTEO':False,'STEFANO':False}
...
print(np.shape(reps))
roi = face_cascade.detectMultiScale(frame,scaleFactor=1.2, minNeighbors=2) 
faces = -1
new_rep = []
for (x,y,w,h) in roi: 
    faces += 1
    ...
    cv2.imwrite('face_' + str(faces) + '.jpg', face)
    iterator = -1
    new_rep[faces] = vgg_face_descriptor.predict(preprocess_image('./face_' + str(faces) + '.jpg'))[0,:]
    for key, value in people.items():
        iterator +=1
        cosine_similarity = findCosineDistance(reps[iterator], new_rep[faces-1])    
        if cosine_similarity < epsilon:
