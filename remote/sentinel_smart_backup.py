...
people = {'DAVID':False, 'STEFANO':False,'ROBERTA':False,'MARIO':False}
...
for (x,y,w,h) in roi: 
    ...
    for key, value in people.items():
        value, distance = verifyFace('./faces/' + key + '_face.jpg','./face.jpg')
        if value == True:
            
    isDavid = True
    if isDavid == True:
        cv2.putText(frame, "David {}".format(distance), (x,y+10), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2)
    else: 
        cv2.putText(frame, "Unknown {}".format(distance), (x,y+10), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2)
entered = False
...
while True:
    ...
    if success:
        ...
        if count%150 == 0:
            ...
            face = frame[y:y+h, x:x+w]
            cv2.imwrite("face.jpg", face)
            isDavid, distance = verifyFace('./faces/DAVID_face.jpg','./face.jpg')
            print(isDavid)
        if entered == True:
            if isDavid == True:
                cv2.putText(frame, "David {}".format(distance), (x,y+10), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2)
